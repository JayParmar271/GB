...
%>
		...
		<input type="hidden" name="business" value="jayparmar271@gmail.com">
		<input type="hidden" name="currency_code" value="US">
		 <%
							try{ 
								...
								String sql = "SELECT * FROM products AS p INNER JOIN cart AS c ON p.p_id=c.p_id INNER JOIN category AS cat ON p.p_cat_id=cat.cat_id INNER JOIN subcategory AS sc ON sc.sub_id=p.p_subcat_id INNER JOIN company AS com ON com.com_id=p.p_com_id WHERE u_id = '"+ userid +"' AND cat_name='"+ category +"'";
								resultSet = statement.executeQuery(sql);
								int count=1;
								while(resultSet.next()){
								String price = resultSet.getString("p_price");
								int finalprice = Integer.parseInt(price)/65;
						%>
		<input type="hidden" name="item_name_<%=count %>" value="<%=resultSet.getString("p_title") %>">
		<input type="hidden" name="amount_<%=count %>" value="<%=finalprice %>">
						<% 
		count = count + 1;
		}
		...
		<center>
			<a href="insert_verify.jsp"><input type="button" class="btn btn-primary" value="Apply For Verification"></button></a><br>
			<input type="submit" class="btn btn-primary" value="Buy Insurance" src="http://www.paypal.com/en_US/i/btn/x-click-but01.gif" name="submit" alt="Make payments with PayPal - it's fast, free and secure!"><br>
			<input type="button" class="btn btn-primary" value="Apply For Loan">
		</center>
		</form>
